...
alldata <- read.csv("2-Cleaned_data/cleaned_df.csv") #all obs from 2024 and 2025
...
# Here we are testing if there is an association between tree species and DBH class
# If yes, that means certain species tend to occur in certain size classes, which
# means we should not have them both as fixed effects in our model because 

dbh_species_table <- table(alltrees_dbhclass$Species.Code,
                           alltrees_dbhclass$DBH_class)

dbh_species_table
chisq.test(dbh_species_table)



cramers_v(dbh_species_table)
...
alldata_filter <- alldata_dbhclass %>% 
